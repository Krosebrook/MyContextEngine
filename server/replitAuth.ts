...
export function getSession() {
  ...
  return session({
    ...
    cookie: {
      ...
      maxAge: sessionTtl,
      sameSite: isProduction ? "none" : "lax", // Adjust for dev/prod
    },
  });
}
...
export async function setupAuth(app: Express) {
  ...
  // Configure strategies for all known domains
  ...
  // Add fallback strategy for local development
  const localStrategy = new Strategy(
    {
      name: "replitauth:local",
      config,
      scope: "openid email profile offline_access",
  ...
  app.get("/api/login", (req, res, next) => {
    // Use hostname-specific strategy if available, otherwise use local strategy
